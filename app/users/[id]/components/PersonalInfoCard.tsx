...
export default function PersonalInfoCard({ user, isLoading }: PersonalInfoCardProps) {
    return (
        <Card className="lg:col-span-2 rounded-3xl border-border shadow-sm overflow-hidden" styles={{ body: { padding: 0 } }}>
            <div className="bg-background px-6 py-4 border-b border-border flex items-center gap-2">
                <IdCard size={18} className="text-text-info" />
...
function InfoSection({ label, value, loading, icon, copyValue }: { label: string; value?: React.ReactNode; loading?: boolean; icon?: any; copyValue?: string }) {
    ...
    return (
        <div className="space-y-1.5 min-h-[46px] group">
            <span className="text-[11px] font-bold text-text-info uppercase tracking-wider flex items-center gap-1.5">
                {icon}
                {label}
            </span>
            {loading ? (
                ...
                <div className="flex items-center gap-2">
                    <p className="text-[15px] font-semibold text-text truncate">{value || 'N/A'}</p>
                    {copyValue && value && (
                        <Tooltip title={copied ? 'Copied!' : `Copy ${label}`}>
                            <button
                                onClick={handleCopy}
                                className="p-1 hover:bg-gray-100 rounded-md text-text-info hover:text-primary transition-colors"
                            >
